...
title: Disarmed and Dangerous
---
# {{ page.meta.module }}: {{ page.meta.title }}

[Murderbot](murderbot-v2.md) hacks into a maintenance terminal, but is warned and expelled.
While exploring, an airlock door closes on and destroys [Murderbot](murderbot-v2.md)'s arm.
The crew discover that Chad and "Steve" were killed by small arms fire, and Tatiana is missing.
[Murderbot](murderbot-v2.md) takes an arm from an android corpse and is able to install it with [Ink](ink.md)'s help.
A brief rest is interrupted by flashing red lights and an intruder warning.
[Carnoc](carnoc-ashbrow.md) and [Dex](dex-miro.md) investigate, witness a nearby firefight, and retreat to warn the others.
As a squad of 4 androids attacks the crew, [Carnoc](carnoc-ashbrow.md) is wounded but still in action.
[Zeke](zeke-sinclair.md) accidentally damages the android scanner, but recovers to team up with [Noriko](noriko.md) to defeat an android.
The crew are able to finish off the other androids, but barely have time to regroup as another squad approaches.
